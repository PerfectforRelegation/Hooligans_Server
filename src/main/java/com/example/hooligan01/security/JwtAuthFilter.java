...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(accessToken != null) {
            ...
            if (jwtUtil.tokenValidation(accessToken)) {
                setAuthentication(jwtUtil.getAccountFromToken(accessToken));
            }
            // 어세스 토큰이 만료된 상황 && 리프레시 토큰 또한 존재하는 상황
                ...
                if (isRefreshToken) {
                    ...
                    String loginId = jwtUtil.getAccountFromToken(refreshToken);
                    // 새로운 어세스 토큰 발급
                    String newAccessToken = jwtUtil.createToken(loginId, "Access");
                    // 헤더에 어세스 토큰 추가
                    jwtUtil.setHeaderAccessToken(response, newAccessToken);
                    // Security context에 인증 정보 넣기
                    setAuthentication(jwtUtil.getAccountFromToken(newAccessToken));
    ...
    public void setAuthentication(String account) {
        Authentication authentication = jwtUtil.createAuthentication(account);
    ...
    public void jwtExceptionHandler(HttpServletResponse response, String msg, HttpStatus status) {
        ...
        try {
            String json = new ObjectMapper().writeValueAsString(status.value());
